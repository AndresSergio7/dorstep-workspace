...
export default function MeetingsPage() {
  ...
  useEffect(() => {
    supabase.from('meetings').select('*, client:clients(id, name, company)').order('date', { ascending: false }).then(({ data }) => {
      setMeetings(data ?? [])
      setLoading(false)
    })
  }, [])
  ...
  const clientOptions = useMemo(() => {
    ...
    for (const m of meetings) {
      if (m.client && !seen.has(m.client.id)) { seen.add(m.client.id); opts.push({ id: m.client.id, name: m.client.name }) }
    }
    ...
    <AppLayout>
      <div className="max-w-4xl">
        <div className="flex items-center justify-between mb-6">
          <div><h1 className="page-title">Reuniones</h1><p className="text-slate-500 text-sm mt-1">{meetings.length} reuniones</p></div>
        ...
        <div className="flex gap-3 mb-5 flex-wrap">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input className="input pl-9 pr-9 text-sm w-full" placeholder="Buscar por título o cliente..." value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <div className="relative min-w-[200px]">
            ...
            <select className="input text-sm pr-8 appearance-none w-full" value={selectedClient} onChange={e => setSelectedClient(e.target.value)}>
              <option value="">Todos los clientes</option>
              {clientOptions.map(c => (<option key={c.id} value={c.id}>{c.name}</option>))}
            </select>
          </div>
          {hasFilter && (<button onClick={clearFilters} className="btn-secondary text-sm flex items-center gap-1.5 whitespace-nowrap"><X size={13} />Limpiar</button>)}
        </div>
        {loading ? (
            ...
            {filtered.map((m: any) => (
              <Link key={m.id} href={`/meetings/${m.id}`} className="card hover:shadow-md transition-all hover:border-blue-200 flex items-center justify-between group">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg bg-teal-50 flex items-center justify-center flex-shrink-0"><CalendarDays size={18} className="text-teal-600" /></div>
                  <div>
